...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        # mm-prefix (Gemma 3/4 multimodal image-span) grouping is not wired
        # up here; only the DiffusionGemma per-request-causal grouping is.
        ...
        # trtllm-gen backend applies identically), unlike the backend="auto"
        # DFlash _noncausal_prefill_wrapper which rejects nvfp4.
    ...
    def get_q_data_type(self, is_prefill: bool) -> torch.dtype:
        ...
        if cache_dtype.startswith("nvfp4"):
            # trtllm-gen NVFP4 KV uses FP8 queries; the FA2 paged path
            # (sm12x) uses model-dtype queries.
    ...
    def _get_group_prefill_wrapper(
        ...
        # nvfp4-aware construction (the NVFP4 jit module is applied at
        # plan()/run() time). Only reachable on the grouped path (no DCP).
        ...
        * ``causal`` is the per-request causal flag (DiffusionGemma:
        ...
        for group_mm, group_causal in (
            ...
            req_indices = torch.tensor(
                [i for i in range(num_prefills) if keys[i] == (group_mm, group_causal)],
                dtype=torch.int64,
            )
    ...
    def build(
        ...
        route_decode = causal_dispatch or self.use_dedicated_xqa
        if route_decode:
        ...
        decode_with_flashinfer_trtllm_api = self.use_trtllm_decode_attention and (
            causal_dispatch or self.use_dedicated_xqa
        )
                ...
                if self.use_dcp:
                    ...
                    if prefill_mm_spans is not None or per_request_causal:
                        ...
                        if per_request_causal:
                            ...
                            causal_prefill_cpu = (
                                causal[prefill_start:num_reqs].cpu().bool()
                            )
...
class FlashInferImpl(AttentionImpl):
    ...
    def forward(
        ...
        if self.is_kvcache_nvfp4:
            if self.use_fa2_nvfp4_kv:
                # Consumer Blackwell (sm120/sm121): get_kv_cache_shape() packs
                # K into heads [0:num_kv_heads) and V into heads
                # [num_kv_heads:2*num_kv_heads) of the raw (unpermuted) cache.
                # Reshape the head axis into (kv=2, num_kv_heads) — a zero-copy
                # view — then split via the contiguous [all-data|all-SF]
                # layout the FA2 paged nvfp4 kernel requires.
                kv_cache_5d = kv_cache.reshape(
                    kv_cache.shape[0], 2, self.num_kv_heads, *kv_cache.shape[2:]
                )
                nvfp4_kv_data, nvfp4_kv_block_scales = nvfp4_kv_cache_split_views(
                    kv_cache_5d, contiguous_sf_layout=True
                )
            else:
        ...
        if num_prefill_tokens > 0:
            ...
            if not prefill_use_trtllm:
                ...
                if use_dcp:
                    ...
                    if isinstance(
                    ...
                    elif prefill_groups is not None:
                        ...
                        # groups interleave within the batch.
                        if self.is_kvcache_nvfp4:
