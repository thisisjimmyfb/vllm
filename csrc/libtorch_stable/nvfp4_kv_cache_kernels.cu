...
void reshape_and_cache_nvfp4_dispatch(
  ...
  if (contiguous_layout) {
    ...
    // dim 1; the per-block stride spans both sides so sf_stride ==
    // data_stride/8 holds for each side (matches nvfp4_kv_cache_split_views
    // global split).
        ...
        if (kv_cache_dtype == "nvfp4") {
          vllm::reshape_and_cache_nvfp4_kernel<
              ...
              <<<grid, block, 0, stream>>>(
                  ...
                  slot_mapping.const_data_ptr<int64_t>(), k_scale_ptr,
                  v_scale_ptr, key.stride(0), value.stride(0), num_heads,
                  head_size, block_size, data_block_stride, data_head_stride,
                  data_block_offset_stride, scale_block_stride,
