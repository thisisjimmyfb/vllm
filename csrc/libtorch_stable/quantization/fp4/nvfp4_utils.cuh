...
#pragma unroll
  ...
  float2 const scale2 = make_float2(outputScale, outputScale);
  ...
  #pragma unroll
    for (int i = 0; i < CVT_FP4_ELTS_PER_THREAD / 4; i++) {
      fp2Vals[i] = __fmul2_rn(cast_to_float2(vec.elts[i]), scale2);
    }
  ...
  #pragma unroll
    for (int i = 0; i < CVT_FP4_ELTS_PER_THREAD / 4; i++) {
      fp2Vals[i] = __fmul2_rn(
          cast_to_float2(vec.elts[i + CVT_FP4_ELTS_PER_THREAD / 4]), scale2);
    }
